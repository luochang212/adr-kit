...
Add `decided-by: human | machine` to the record front matter, so a durable
record states whether a person or a machine initiated the decision. No author
name or account is recorded: git already owns identity, and the field covers
only the axis git cannot answer, because an agent session commits as the human
user.
...
- `adrkit supersede` preserves the retiring record's value instead of
  re-stamping it: the field states who made the decision, not who rewrote the
  file.
...
inferred stamp rather than proof, and names what defeats it.
