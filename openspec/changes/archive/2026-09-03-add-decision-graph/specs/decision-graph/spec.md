...
The graph SHALL draw one solid edge per `superseded-by` front matter field,
from the superseded decision to its replacement, labeled as a supersession.

#### Scenario: supersede edge is drawn

- **WHEN** decision 1 carries `superseded-by: 11`
- **THEN** the output contains a solid edge from node 1 to node 11 with a
  supersession label
