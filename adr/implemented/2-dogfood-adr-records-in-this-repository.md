...
rule in `AGENTS.md` tells agents to read `adr/` decisions. An implemented
record only one machine can see is not a record.

## Decision

This repository dogfoods its own records. `adr/` is tracked: the root
`.gitignore` no longer ignores it, and ADR 1 is committed rather than left
...
- **Keep `adr/` local and delete the reading rule**: rejected. It resolves the
  ...
  worth adopting: an implemented decision would still have no durable home.
...
- `adr/implemented/1-…md` and `2-…md` become committed history; CI and every
  clone see the same decisions the maintainer sees.
- `adrkit validate` is now a real gate on this repository's own changes.
- ADR 18 later moved the committed decisions to `adr/implemented/` and
  replaced the ephemeral `.drafts/` with `adr/proposed/`.
